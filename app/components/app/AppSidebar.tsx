    ...
    <Sidebar side="left" className="h-screen border-none">
      <SidebarContent className="h-full flex flex-col p-2 justify-between bg-dracula-background/50">
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu>
              <SidebarMenuItem className="flex items-center justify-between mt-4">
                <Link
                  to="/"
              ...
              <SidebarMenuItem className="flex flex-col gap-3 mt-12">
                {menuItems.map((item) => (
                  <NavLink
                    to={item.path}
                    key={item.name}
                    className={({ isActive }) =>
                      `w-full text-xl flex items-center px-6 gap-4 py-4 cursor-pointer rounded-xl 
                      ${
                        isActive
